...
Viewer <- function(..., plottype = "Both", fullfile = "", ifile = "", linlog = "Log") {
  ...
  if (nff > 0) {
    conk <- fullfile # option for use by Meta.R
  } else if (nff == 0) {
    ...
    # read selected file
    cat("file read in ", (proc.time() - tglo)[3], " s", sep = "")
  ...
  if (enviid == 1) {
    envi <- "Air"
    pref <- 20 # assign PAMGuide variables envi, calib, atype from metadata
  } else {
    envi <- "Wat"
    pref <- 1
  }
  ...
  if (tstamp == 1 & t[1] < 1e7) {
    t <- (t - 719529) * 86400 # convert from MATLAB time if so formatted
  ...
  if (plottype == "Time" | plottype == "Both") {
    ...
    if (atype == "PSD") { # PSD spectrogram
      if (linlog == "Log") {
        if (tstamp != "") {
          image(t, f, a, ylim = c(min(f), max(f)), yaxt = "n", log = "y", col = jet.colors(512), 
            main = paste("PSD spectrogram of ", ifile), xlab = "Time", 
            ylab = "Frequency [ Hz ]", xaxt = "n")
        } else {
          image(t, f, a, ylim = c(min(f), max(f)), yaxt = "n", log = "y", col = jet.colors(512), 
            main = paste("PSD spectrogram of ", ifile), xlab = "Time [ s ]", 
            ylab = "Frequency [ Hz ]")
        }
        ...
        if (tstamp != "") {
          image(t, f, a, ylim = c(min(f), max(f)), col = jet.colors(512), 
            main = paste("PSD spectrogram of ", ifile), xlab = "Time", 
            ylab = "Frequency [ Hz ]", xaxt = "n")
        } else {
          image(t, f, a, ylim = c(min(f), max(f)), col = jet.colors(512), 
    ...
    if (atype == "PowerSpec") { # Power spectrum spectrogram
      if (linlog == "Log") {
        if (tstamp != "") {
          image(t, f, a, ylim = c(min(f), max(f)), yaxt = "n", log = "y", col = jet.colors(512), 
            main = paste("Power spectrum spectrogram of ", ifile), 
            xlab = "Time", ylab = "Frequency [ Hz ]", xaxt = "n")
        }
        else {
          image(t, f, a, ylim = c(min(f), max(f)), yaxt = "n", log = "y", col = jet.colors(512), 
            main = paste("Power spectrum spectrogram of ", ifile), 
    ...
    if (atype == "TOLf") { # Third octave level spectrogram
      if (tstamp != "") {
        image(t, f, a, ylim = c(min(f) * 10^-0.05, max(f) * 10^0.05), 
          yaxt = "n", log = "y", col = jet.colors(512), 
          main = paste("TOL spectrogram of ", ifile), 
          xlab = "Time", ylab = "Frequency [ Hz ]", xaxt = "n")
      }
      else {
        image(t, f, a, ylim = c(min(f) * 10^-0.05, max(f) * 10^0.05), 
    ...
    if (atype == "Broadband") { # Broadband level
      if (tstamp != "") {
        plot(t, a, type = "l", xlab = "Time", ylab = "", xaxt = "n")
      }
      else {
        plot(t, a, type = "l", xlab = "Time [ s ]", ylab = "", main = paste("Broadband SPL of ", ifile))
      }
      if (envi == "Air") {
        mtext(text = expression(paste("Broadband SPL [ dB re 20 ", mu, "Pa ]")), side = 2, line = 2)
      }
      if (envi == "Wat") {
        mtext(text = expression(paste("Broadband SPL [ dB re 1 ", mu, "Pa ]")), side = 2, line = 2)
      }
    }
    ...
    if (atype == "Waveform") { # Waveform
      x <- a
      if (tstamp != "") {
        t <- t + tstamp # if time stamp provided
        ...
        plot(t, a / 1e6, xlab = "Time", ylab = "Pressure [ Pa ]", 
          type = "l", ylim = c(-max(x / 1e6), max(x / 1e6)), xaxt = "n")
        tck <- axis(1, labels = FALSE)
        axis.POSIXct(1, as.POSIXct.numeric(tck, origin = "1970-01-01", tz = "UTC"), 
          format = tform, label = TRUE)
      }
      else {
        plot(t, x / 1e6, xlab = "Time [ s ]", ylab = "Pressure [ Pa ]", 
          type = "l", ylim = c(-max(x / 1e6), max(x / 1e6)))
      }
    }
    ...
    if (atype != "Waveform") {
      if (plottype == "Stats" | plottype == "Both") {
        if (atype != "Broadband") {
          ...
          if (M >= 1000) {
            ...
            if (linlog == "Log") {
              image(f, dbvec[1:length(dbvec) - 1] - hind / 2, d, 
                zlim <- c(min(d[which(d > 0)]), 0.05), log = "x", xaxt = "n", 
                col = jet.colors(2^12), xlim <- c(min(f), max(f)), 
          ...
          if (M < 1000) {
            legend("topright", c("99%", "95%", "50%", "5%", "1%", "RMS level"), lty = c(1), 
              lwd = 2, col = c("black", "gray20", "gray40", "gray60", "gray80", "magenta"))
          }
          if (M >= 1000) {
            legend("topright", c("SPD", "99%", "95%", "50%", "5%", "1%", "RMS level"), lty = c(1), 
              lwd = 2, col = c("blue", "black", "gray20", "gray40", "gray60", "gray80", "magenta"))
          }
          if (linlog == "Log") {
            y1 <- floor(log10(range(f)))
            pow <- seq(y1[1], y1[2] + 1)
          ...
          if (atype == "PSD") {
            if (calib == 1) {
              if (envi == "Air") {
                YLAB <- expression(paste("PSD [ dB re 20 ", mu, "Pa"^2, "Hz"^{
                  -1
                }, "]"))
              } else {
                YLAB <- expression(paste("PSD [ dB re 1 ", mu, "Pa"^2, "Hz"^{
                  -1
                }, "]"))
              }
            } else {
              YLAB <- expression(paste("Relative PSD [ dB ]"))
            }
          ...
          if (atype == "PowerSpec") {
            if (calib == 1) {
              if (envi == "Air") {
                YLAB <- expression(paste("Power spectrum [ dB re 20 ", mu, "Pa"^2, "Hz"^{
                  -1
                }, "]"))
              } else {
                YLAB <- expression(paste("Power spectrum [ dB re 1 ", mu, "Pa"^2, "Hz"^{
                  -1
                }, "]"))
              }
            } else {
              YLAB <- expression(paste("Relative power spectrum [ dB ]"))
            }
            mtext(text = YLAB, side = 2, line = 2)
          }
          if (atype == "TOLf") {
            if (calib == 1) {
              if (envi == "Air") {
                YLAB <- expression(paste("SPL [ dB re 20 ", mu, "Pa ]"))
              } else {
                YLAB <- expression(paste("SPL [ dB re 1 ", mu, "Pa ]"))
              }
            } else {
              YLAB <- expression(paste("Relative SPL [ dB ]"))
            }
            mtext(text = YLAB, side = 2, line = 2)
          }
...
")
  ...
  if (atype == "Broadband") {
    if (plottype == "Stats" | plottype == "Both") {
      ...
      dev.new() # open new plot window
      plot(p, c(0:100) / 100, type = "n", xlab = "", 
        ylab = "Cumulative Distribution Function", main = paste("CDF of broadband SPL for ", ifile))
      lines(p, c(0:100) / 100)
      if (calib == 1) {
        if (envi == "Air") {
          XLAB <- expression(paste("Broadband SPL [ dB re 20 ", mu, "Pa ]"))
        } else {
          XLAB <- expression(paste("Broadband SPL [ dB re 1 ", mu, "Pa ]"))
        }
      } else {
        XLAB <- expression(paste("Relative SPL [ dB ]"))
      }
...
")
    ...
    if (plottype == "Stats" | plottype == "Both") {
      ...
      legend("bottomright", c("CDF", "RMS level", "Median", "Mode"), 
        lty = c(1), lwd = 2, col = c("black", "magenta", "green", "blue"))
    }
  }
}
