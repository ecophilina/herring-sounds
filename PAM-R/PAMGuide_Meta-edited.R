#-------PAMGuide_Meta-edited.R
# modified from Meta.R to handle our date format and source from a subfolder

# Function called by Meta-edited.R for batch processing of passive acoustic data.
...
PAMGuide_Meta <- function(fullfile, ..., atype = "PSD", plottype = "Both", envi = "Air", calib = 0, ctype = "TS", Si = -159, Mh = -36, G = 0, vADC = 1.414, r = 50, N = Fs, winname = "Hann", lcut = Fs / N, hcut = Fs / 2, timestring = "", outdir = dirname(fullfile), outwrite = 0, disppar = 1, welch = "", tones = F, chunksize = "", linlog = "Log") {
  ...
  if (calib == 0) {
    aid <- aid + 20 # add calibration element to metadata code
  } else {
    aid <- aid + 10
  }
  if (timestring != "") {
    ...
    aid <- aid + 2000
  } # add time stamp element to metadata code
...
")
    ...
    tstamp <- NA
  } # compute time stamp in R POSIXct format
...
")
  ...
  for (q in 1:nchunks) {
    ...
    if (envi == "Air") {
      pref <- 20
      aid <- aid + 100
    } # set reference pressure depending for in-air or underwater
    if (envi == "Wat") {
      pref <- 1
      aid <- aid + 200
    }
...
")
  ...
  if (plottype == "Time" | plottype == "Both") {
    ...
    if (atype == "Broadband") {
      if (calib == 1) {
        if (envi == "Air") {
          YLAB <- expression(paste("Broadband SPL [ dB re 20 ", mu, "Pa ]"))
        } else {
          YLAB <- expression(paste("Broadband SPL [ dB re 1 ", mu, "Pa ]"))
        }
      } else {
      ...
      if (tstamp != "") {
        ...
        plot(t, a, type = "l", xlab = "Time [ s ]", ylab = "")
      }
      if (envi == "Air") {
    ...
    if (atype == "Waveform") {
      ...
      if (tstamp != "") {
        ...
        plot(t, a / 1e6, xlab = "Time [ s ]", ylab = YLAB, type = "l", ylim = c(-max(a / 1e6), max(a / 1e6)))
      }
    }
