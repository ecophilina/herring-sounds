...
PAMMeta <- function(..., atype = "PSD", plottype = "Both", envi = "Air", calib = 0, ctype = "TS", Si = -159, Mh = -36, G = 0, vADC = 1.414, r = 50, N = Fs, winname = "Hann", lcut = Fs / N, hcut = Fs / 2, timestring = "", outdir = dirname(fullfile), outwrite = 0, disppar = 1, welch = "", chunksize = "", linlog = "Log") {
  # define Meta function

  source("PAM-R/PAMGuide_Meta-edited.R") # load PAMGuide_Meta function
  source("PAM-R/Viewer.R") # load Viewer function
...
")
  ...
  for (i in 1:nf) { # loop through files and analyse
    ...
    A <- PAMGuide_Meta(nowfile, atype = atype, plottype = "None", envi = envi, calib = calib, ctype = ctype, stype = stype, Si = Si, Mh = Mh, G = G, vADC = vADC, r = r, N = N, winname = winname, lcut = lcut, hcut = hcut, timestring = timestring, outdir = newpath, outwrite = outwrite, disppar = disppar, welch = welch, chunksize = chunksize, linlog = linlog)
