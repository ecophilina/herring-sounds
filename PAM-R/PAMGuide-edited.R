#-------PAMGuide-edited.R
# modified from PAMGuide.R to handle our date format and source from a subfolder
...
PAMGuide <- function(...,
                     ...
                     vADC = 1.414,
                     r = 50, # % window overlap
                     N = Fs, # default Fs is the sampling rate in Hz = 96000 in our data
                     winname = "Hann",
                     lcut = Fs / N, # defaults to 1 Hz
                     hcut = Fs / 2, # defaults to max possible (eg. 48 kHz when sampling rate = 96000 Hz)
                     timestring = "",
                     outdir = dirname(fullfile),
                     outwrite = 0,
                     disppar = 1,
                     welch = "",
                     chunksize = "",
...
")
      ...
      S <- 0
    } # if not calibrated (calib == 0), S is zero
    ...
    if (atype != "Waveform") {
      if (nchunks == 1) {
        ...
        if (atype == "PowerSpec") {
          cat("Computing power spectrum...")
        }
        if (atype == "TOL") {
          cat("Computing TOLs by DFT method...")
        }
        if (atype == "Broadband") {
          cat("Computing broadband level...")
        }
        tana <- proc.time()
      }
...
")
  ...
  if (!is.na(tstamp)) {
    ...
    if (tdiff > 10 & tdiff < 86400) {
      tform <- "%H:%M:%S"
    }
    if (tdiff > 86400 & tdiff < 86400 * 7) {
      tform <- "%H:%M \n %d %b"
    }
    if (tdiff > 86400 * 7) {
      tform <- "%d %b %y"
...
")
  ...
  source("PAM-R/Viewer.R") # initialise Viewer
