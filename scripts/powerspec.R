...
# set_welch <- 40 # 20 sec time resolution if N = 96000 (default)
# set_welch <- 120 # 1 min time resolution if N = 96000 (default)
set_welch <- 72*7.5 # for 7.5 min resolution, if setting r = 0 if N = 96000 (default)

# calculate welch for N = 1024, that results in 7.5min resolution
# 72*7.5 = x * (1024/96000)
# x= 72*7.5/(1024/96000)
# x
set_N <- 1024
set_welch <- 50625 # for 7.5 min resolution, if setting r = 0 if N = 96000
...
#   atype = "PowerSpec",# this tells it you want to do power spectral density
...
PAMMeta(
  ...
  ctype = "EE",# type of calibration, for soundtraps it is end to end
  Si = calib_value,# calibration value from the ocean instruments site
  N = set_N, #96000hz is default... dft = 96000/N
  lcut = set_lcut,# low frequency cut off on Hz
  hcut = 18000,# high frequency cut off on Hz
  welch = set_welch,# assuming default of 50% overlap, this is the # seconds x2
  r = 0.01, # % window overlap
  tones = T,
  plottype = "none",# tells it whether or not to plot the output
...
d3 <- d2 %>% 
  pivot_longer(
    cols = 1:(length(dt)), names_to = "datetime", values_to = "Power"
  ) %>% mutate(
...
#   annotate("text", x = min(ds$time)+set_welch/2*6, y = 3, label = "herring sounds") +
...
d5 <- d3 %>% group_by(group) %>% mutate(
  ...
  time_elapsed = as.factor((((t - min(t))/60)/60)+3)
)

## using time elapsed
d5 %>% filter(freq_kHz > 0.2 & freq_kHz < 24) %>%
ggplot(aes(time_elapsed, freq_kHz)) +
  geom_rect(aes(xmin=as.numeric(time_elapsed), xmax=as.numeric(time_elapsed)+1,
    ymin=freq_kHz-(96000/set_N/1000), ymax=freq_kHz+(96000/set_N/1000), 
    fill=Power)) +
...
d5 %>% filter(freq_kHz > 0.2 & freq_kHz < 20) %>%
